...
Guild.prototype.log = async function log(admin, user, document, channelId) {
    ...
    if (channel) {
        let embed = new MessageEmbed()
            ...
            .setColor("RANDOM")
            .setDescription(`${admin} **Tarafından ${user} Kişisi** **| ${document.Reason} |** **Sebebi ile ${document.Type} cezası aldı.** (**Ceza NO:** #${document.Id})`)
            .addField("__Ceza Bilgisi:__", `
        **Ceza Numarası:** \`#${document.Id}\`
        **Ceza Tipi:** __${document.Type}__
        `)
            .addField("__Yetkili ve Kullanıcı :__", `
        **__Yetkili__:** ${admin}(${admin.id})
