...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    const sehira = await message.reply({components: [Row] ,embeds: [new genEmbed().setDescription(`:tada: Aşağıdaki kategorilerden komut yardımı almak istediğiniz kategoriyi seçin!`)]})
    var filter = i => i.user.id == message.member.id
    let collector = sehira.createMessageComponentCollector({filter: filter, time: 30000, error: ["time"]})
    
    collector.on("collect", i => {
        if(i.customId == "yardimmenusu") {
           if(i.values == "talent") {
                sehira.edit({embeds: [new genEmbed().setDescription(`${Data ?  Data.talentPerms ? Data.talentPerms.map(x => `\`${sistem.botSettings.Prefixs[0] + x.Commands + " <@sehira/ID>"}\``).join("\n") : '' : ''}`)]})
           } else {
                sehira.edit({embeds: [new genEmbed().setDescription(`${client.commands.filter(x => x.Extend != false && x.Kategori == `${i.values}`).map(x => `\`${sistem.botSettings.Prefixs[0] + x.Kullanim}\``).join("\n")}`)]})
           }
           i.deferUpdate()
        }
    })
    collector.on("end", () => {
        sehira.edit({embeds: [new genEmbed().setDescription(`${client.commands.filter(x => x.Extend != false).map(x => `\`${sistem.botSettings.Prefixs[0] + x.Kullanim}\``).join("\n")}
        \`${Data ?  Data.talentPerms ? Data.talentPerms.map(x => sistem.botSettings.Prefixs[0] + x.Commands + " <@sehira/ID>").join("\n") : '' : ''}\``)], components: []})
      })
    }
};